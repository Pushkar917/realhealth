...
class CustomUserManager(BaseUserManager):
    ...
    def create_user(self, email, username, first_name, last_name, password, **extra_fields):
        ...
        if not first_name:
            raise ValueError("You must enter a first_name")

        if not last_name:
            raise ValueError("You must enter a last_name")
        ...
        if email:
            ...
            self.email_validator(email)
             ...
             raise ValueError('Base User Account: You must provide a email address')
